...
public class GWRPropreitaryActivity extends AppCompatActivity {

    private static final String TAG = "GWRPropreitaryActivity";
    Toolbar toolbar;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gwr_propreitary);

        this.context = GWRPropreitaryActivity.this;
        connection = new Connection(getApplicationContext());

        toolbar = (Toolbar) findViewById(R.id.toolbarPropreitary);
        setSupportActionBar(toolbar);
        toolbar.setTitleTextColor(Color.WHITE);
        if (getSupportActionBar() != null) {
            getSupportActionBar().setDisplayHomeAsUpEnabled(true);
            getSupportActionBar().setDisplayShowHomeEnabled(true);
            String title = "Preparatory Activity";
            getSupportActionBar().setTitle(Html.fromHtml("<small>" + title + "</small>"));
        }

        final TabLayout tabLayout = (TabLayout) findViewById(R.id.tab_propreitary);

        tabLayout.addTab(tabLayout.newTab().setText("Pre-requisite"));
        tabLayout.addTab(tabLayout.newTab().setText("Witness Profile"));
        tabLayout.addTab(tabLayout.newTab().setText("Cameraman Engagement"));
        ...
        viewPager = (CustomViewPager) findViewById(R.id.propreitary_pager);
        viewPager.setOffscreenPageLimit(3);
        final GWRPropreitaryViewPager adapter = new GWRPropreitaryViewPager(GWRPropreitaryActivity.this, getSupportFragmentManager(), tabLayout.getTabCount(), IsEditable);
