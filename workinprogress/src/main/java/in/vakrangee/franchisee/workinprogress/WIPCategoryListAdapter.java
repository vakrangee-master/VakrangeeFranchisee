...
public class WIPCategoryListAdapter extends RecyclerView.Adapter<WIPCategoryListAdapter.MyViewHolder> {

    private static final String TAG = "WIPCategoryListAdapter";
    private List<WIPInteriorCategoryDto> wipCategoryList;
    private Context context;
    private View itemView;
    private DeprecateHandler deprecateHandler;
    public WIPInteriorCellStatusAdapter interiorChecklistAdapter;
    private WIPCategoryListAdapter.ISiteCheckHandler iSiteCheckHandler;
    ...
    public WIPCategoryListAdapter(Context context, Activity activity, boolean IsFranchisee, String type, List<WIPInteriorCategoryDto> categoryList, WIPCategoryListAdapter.ISiteCheckHandler iSiteCheckHandler) {
        this.context = context;
        this.activity = activity;
        this.IsFranchisee = IsFranchisee;
        this.ATTRIBUTE_TYPE = type;
        this.wipCategoryList = categoryList;
    ...
    @Override
    public WIPCategoryListAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_interior_category, parent, false);
        return new WIPCategoryListAdapter.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final WIPCategoryListAdapter.MyViewHolder holder, int position) {

        final WIPInteriorCategoryDto dto = wipCategoryList.get(position);
        holder.txtCategoryName.setText(dto.getName());

        //Check list Recycler view
        interiorChecklistAdapter = new WIPInteriorCellStatusAdapter(context, activity, IsFranchisee, ATTRIBUTE_TYPE, position, dto.checkList, new WIPInteriorCellStatusAdapter.ISiteCheckHandler() {
            @Override
            public void cameraClick(int parentPosition, int position, WIPCheckListDto checkListDto) {
                iSiteCheckHandler.cameraClick(parentPosition, position, checkListDto);
            }
        });
