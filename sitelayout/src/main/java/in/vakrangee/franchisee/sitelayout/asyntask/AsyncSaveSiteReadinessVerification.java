...
public class AsyncSaveSiteReadinessVerification extends AsyncTask<String, Void, String> {

    private final static String TAG = "AsyncSaveSiteReadinessVerification";
    private Context mContext;
    private Logger logger;
    private ProgressDialog progress;
    private AsyncSaveSiteReadinessVerification.Callback callback;
    ...
    public AsyncSaveSiteReadinessVerification(Context context) {
        super();
        this.mContext = context;
        logger = Logger.getInstance(context);
    }

    public AsyncSaveSiteReadinessVerification(Context context, List<SiteReadinessCategoryDto> categoryDtoList, FranchiseeDetails franchiseeDetails, String type, AsyncSaveSiteReadinessVerification.Callback callback) {
        super();
        this.mContext = context;
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            if (categoryDtoList == null) {
                ...
                String dataArray = gson.toJson(categoryDtoList, new TypeToken<ArrayList<SiteReadinessCategoryDto>>() {
                }.getType());
                if (type.equalsIgnoreCase(Constants.SITE_READINESS_BRANDING_TYPE)) {
            ...
            if (isAdhoc) {
                ...
                if (!TextUtils.isEmpty(tmpVkId)) {
                    response = WebService.nextgenSiteReadinessAndVerificationUpdate1(tmpVkId, data, type);
                } else {
                    String enquiryId = CommonUtils.getEnquiryId(mContext);
                    response = WebService.nextgenSiteReadinessAndVerificationUpdate1(enquiryId, data, type);
                }
            }
            return response;
